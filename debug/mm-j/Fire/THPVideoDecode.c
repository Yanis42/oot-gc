/*
    ...
    Language: C89
    Code range: 0x80013854 -> 0x80013D74
*/

#include "types.h"

// size = 0x4, address = 0x80145918
...
} __anon_0x1A373; // size = 0x318

// size = 0x318, address = 0x8010A9D0
static struct OSThread VideoDecodeThread;

// size = 0x1000, address = 0x8010ACE8
...
} __anon_0x1ADF0; // size = 0x3C

typedef struct __anon_0x1AEC8 {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x1AEC8; // size = 0x30

typedef struct __anon_0x1B0F4 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x1B0F4; // size = 0x14

typedef struct __anon_0x1B15C {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x1B15C; // size = 0xC

typedef struct __anon_0x1B1DC {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x1B1DC; // size = 0x10

typedef struct __anon_0x1B296 {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x1B296; // size = 0x10

typedef struct __anon_0x1B34C {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x1B34C; // size = 0xC

typedef struct __anon_0x1B43C {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x1AEC8 header;
    /* 0x06C */ struct __anon_0x1B0F4 compInfo;
    /* 0x080 */ struct __anon_0x1B15C videoInfo;
    /* 0x08C */ struct __anon_0x1B1DC audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x1B296* dispTextureSet;
    /* 0x0FC */ struct __anon_0x1B34C* playAudioBuffer;
    /* 0x100 */ struct __anon_0x1BA44 readBuffer[10];
    /* 0x178 */ struct __anon_0x1B296 textureSet[3];
    /* 0x1A8 */ struct __anon_0x1B34C audioBuffer[3];
} __anon_0x1B43C; // size = 0x1D0

// size = 0x1D0, address = 0x80108060
struct __anon_0x1B43C ActivePlayer;

typedef struct __anon_0x1BA44 {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x1BA44; // size = 0xC

// Range: 0x80013D44 -> 0x80013D74
...
// Range: 0x80013B7C -> 0x80013C9C
static void VideoDecode(struct __anon_0x1BA44* readBuffer) {
    // Parameters
    // struct __anon_0x1BA44* readBuffer; // r24

    // Local variables
    struct __anon_0x1B296* textureSet; // r28
    u32 i; // r27
    u32* compSizePtr; // r26
    u8* ptr; // r25
    s32 old; // r3
...
// Range: 0x80013A48 -> 0x80013B7C
static void* VideoDecoderForOnMemory(void* ptr) {
    // Parameters
    // void* ptr; // r1+0x8

    // Local variables
    struct __anon_0x1BA44 readBuffer; // r1+0x10
    s32 old; // r3
    s32 tmp; // r4
    s32 size; // r29
    s32 readFrame; // r28
...
// Range: 0x8001396C -> 0x80013A48
static void* VideoDecoder() {
    // Local variables
    struct __anon_0x1BA44* readBuffer; // r28
    s32 old; // r3
