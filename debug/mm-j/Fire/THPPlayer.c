/*
    ...
    Language: C89
    Code range: 0x800101C4 -> 0x800116D0
*/

#include "types.h"

// size = 0x100, address = 0x800F6308
static u16 VolumeTable[128];

// size = 0x4, address = 0x801458D8
static s32 Initialized;

// size = 0x40, address = 0x80107AC0
...
typedef struct OSMessageQueue {
    ...
    /* 0x18 */ s32 firstIndex;
    /* 0x1C */ s32 usedCount;
} __anon_0x117C7; // size = 0x20

// size = 0x20, address = 0x80107B00
static struct OSMessageQueue PrepareReadyQueue;

// size = 0x20, address = 0x80107B20
static struct OSMessageQueue UsedTextureSetQueue;

// size = 0x4, address = 0x801458DC
static void* PrepareReadyMessage;

// size = 0xC, address = 0x80107B40
static void* UsedTextureSetMessage[3];

// size = 0x4, address = 0x801458E0
static void (* OldVIPostCallback)(u32);

// size = 0x500, address = 0x80107B60
static s16 SoundBuffer[2][320];

// size = 0x4, address = 0x801458E4
static s32 SoundBufferIndex;

// size = 0x4, address = 0x801458E8
static void (* OldAIDCallback)();

// size = 0x4, address = 0x801458EC
static s16* LastAudioBuffer;

// size = 0x4, address = 0x801458F0
static s16* CurAudioBuffer;

// size = 0x4, address = 0x801458F4
...
} __anon_0x11F5E; // size = 0x3C

typedef struct __anon_0x12036 {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x12036; // size = 0x30

typedef struct __anon_0x12262 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x12262; // size = 0x14

typedef struct __anon_0x122CA {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x122CA; // size = 0xC

typedef struct __anon_0x1234A {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x1234A; // size = 0x10

typedef struct __anon_0x12404 {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x12404; // size = 0x10

typedef struct __anon_0x124BA {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x124BA; // size = 0xC

typedef struct __anon_0x12544 {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x12544; // size = 0xC

typedef struct __anon_0x1262F {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x12036 header;
    /* 0x06C */ struct __anon_0x12262 compInfo;
    /* 0x080 */ struct __anon_0x122CA videoInfo;
    /* 0x08C */ struct __anon_0x1234A audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x12404* dispTextureSet;
    /* 0x0FC */ struct __anon_0x124BA* playAudioBuffer;
    /* 0x100 */ struct __anon_0x12544 readBuffer[10];
    /* 0x178 */ struct __anon_0x12404 textureSet[3];
    /* 0x1A8 */ struct __anon_0x124BA audioBuffer[3];
} __anon_0x1262F; // size = 0x1D0

// size = 0x1D0, address = 0x80108060
struct __anon_0x1262F ActivePlayer;

typedef enum __anon_0x134A9 {
...
typedef enum __anon_0x135F5 {
    VI_XFBMODE_SF = 0,
    VI_XFBMODE_DF = 1,
} __anon_0x135F5;

typedef struct _GXRenderModeObj {
    /* 0x00 */ enum __anon_0x134A9 viTVmode;
    /* 0x04 */ u16 fbWidth;
    /* 0x06 */ u16 efbHeight;
    /* 0x08 */ u16 xfbHeight;
    /* 0x0A */ u16 viXOrigin;
    ...
    /* 0x0E */ u16 viWidth;
    /* 0x10 */ u16 viHeight;
    /* 0x14 */ enum __anon_0x135F5 xFBmode;
    /* 0x18 */ u8 field_rendering;
    /* 0x19 */ u8 aa;
    /* 0x1A */ u8 sample_pattern[12][2];
    /* 0x32 */ u8 vfilter[7];
} __anon_0x136A8; // size = 0x3C
...
// Erased
static s32 THPPlayerGetAudioInfo(struct __anon_0x1234A* audioInfo) {
    // Parameters
    // struct __anon_0x1234A* audioInfo; // r3
...
// Erased
static s32 THPPlayerGetVideoInfo(struct __anon_0x122CA* videoInfo) {
    // Parameters
    // struct __anon_0x122CA* videoInfo; // r3

    // References
    // -> struct __anon_0x1262F ActivePlayer;
}

// Range: 0x80011050 -> 0x8001111C
...
// Range: 0x80010EC0 -> 0x80010F2C
static s32 ProperTimingForStart() {
    // References
    // -> struct __anon_0x1262F ActivePlayer;
}

// Range: 0x80010C24 -> 0x80010EC0
static void PlayControl(u32 retraceCount) {
    ...
    s32 diff; // r1+0x8
    s32 frameNumber; // r4
    struct __anon_0x12404* textureSet; // r29
...
// Range: 0x80010BC8 -> 0x80010C24
s32 THPPlayerPlay() {
    // References
    // -> struct __anon_0x1262F ActivePlayer;
}

// Range: 0x80010954 -> 0x80010BC8
...
// Range: 0x80010858 -> 0x80010924
static void InitAllMessageQueue() {
    // Local variables
    s32 i; // r28
    struct __anon_0x12544* readBuffer; // r3
    struct __anon_0x12404* textureSet; // r3
    struct __anon_0x124BA* audioBuffer; // r3
