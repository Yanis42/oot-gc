/*
    ...
    Language: C++
    Code range: 0x8001226C -> 0x80013854
*/

#include "types.h"

// size = 0x4, address = 0x80145900
static s32 ReadThreadCreated;

typedef struct OSThreadQueue {
    /* 0x0 */ struct OSThread* head;
    /* 0x4 */ struct OSThread* tail;
} __anon_0x16A2F; // size = 0x8
...
} __anon_0x171F4; // size = 0x318

// size = 0x318, address = 0x80109678
static struct OSThread ReadThread;

// size = 0x1000, address = 0x80109990
static u8 ReadThreadStack[4096];

// size = 0x40, address = 0x8010A990
static f32 gOrthoMtx[4][4];

// size = 0x4, address = 0x80145904
s32 gMovieErrorToggle;

// size = 0x4, address = 0x80145908
static s32 toggle$185;

// size = 0x4, address = 0x8014590C
static s32 gbReset;

// size = 0x4, address = 0x80145910
...
} __anon_0x17C20; // size = 0x3C

typedef struct __anon_0x17CF8 {
    /* 0x00 */ char magic[4];
    /* 0x04 */ u32 version;
    /* 0x08 */ u32 bufSize;
...
} __anon_0x17CF8; // size = 0x30

typedef struct __anon_0x17F24 {
    /* 0x0 */ u32 numComponents;
    /* 0x4 */ u8 frameComp[16];
} __anon_0x17F24; // size = 0x14

typedef struct __anon_0x17F8C {
    /* 0x0 */ u32 xSize;
    /* 0x4 */ u32 ySize;
    /* 0x8 */ u32 videoType;
} __anon_0x17F8C; // size = 0xC

typedef struct __anon_0x1800C {
    /* 0x0 */ u32 sndChannels;
    /* 0x4 */ u32 sndFrequency;
    /* 0x8 */ u32 sndNumSamples;
    /* 0xC */ u32 sndNumTracks;
} __anon_0x1800C; // size = 0x10

typedef struct __anon_0x180C6 {
    /* 0x0 */ u8* ytexture;
    /* 0x4 */ u8* utexture;
    /* 0x8 */ u8* vtexture;
    /* 0xC */ s32 frameNumber;
} __anon_0x180C6; // size = 0x10

typedef struct __anon_0x1817C {
    /* 0x0 */ s16* buffer;
    /* 0x4 */ s16* curPtr;
    /* 0x8 */ u32 validSample;
} __anon_0x1817C; // size = 0xC

typedef struct __anon_0x18206 {
    /* 0x0 */ u8* ptr;
    /* 0x4 */ s32 frameNumber;
    /* 0x8 */ s32 isValid;
} __anon_0x18206; // size = 0xC

typedef struct __anon_0x182F1 {
    /* 0x000 */ struct DVDFileInfo fileInfo;
    /* 0x03C */ struct __anon_0x17CF8 header;
    /* 0x06C */ struct __anon_0x17F24 compInfo;
    /* 0x080 */ struct __anon_0x17F8C videoInfo;
    /* 0x08C */ struct __anon_0x1800C audioInfo;
    /* 0x09C */ void* thpWork;
    /* 0x0A0 */ s32 open;
    /* 0x0A4 */ u8 state;
    ...
    /* 0x0F8 */ struct __anon_0x180C6* dispTextureSet;
    /* 0x0FC */ struct __anon_0x1817C* playAudioBuffer;
    /* 0x100 */ struct __anon_0x18206 readBuffer[10];
    /* 0x178 */ struct __anon_0x180C6 textureSet[3];
    /* 0x1A8 */ struct __anon_0x1817C audioBuffer[3];
} __anon_0x182F1; // size = 0x1D0

// size = 0x1D0, address = 0x80108060
struct __anon_0x182F1 ActivePlayer;
...
typedef struct PADStatus {
    ...
    /* 0x9 */ u8 analogB;
    /* 0xA */ s8 err;
} __anon_0x18ABA; // size = 0xC

typedef struct __anon_0x18C4A {
...
} __anon_0x18C4A; // size = 0x1E

// size = 0x78, address = 0x80142458
struct __anon_0x18C4A DemoPad[4];

typedef enum __anon_0x1900C {
    M_M_NONE = -1,
...
// size = 0x0, address = 0x800E79E0
u8 gcoverOpen[];

// size = 0x0, address = 0x800F1940
u8 gwrongDisk[];

// size = 0x0, address = 0x800F3860
u8 greadingDisk[];

// size = 0x0, address = 0x800EC1E0
u8 gretryErr[];

// size = 0x0, address = 0x800EE640
u8 gfatalErr[];

// size = 0x0, address = 0x800EA2C0
u8 gnoDisk[];

// size = 0x0, address = 0x800F5B40
s16 Vert_s16[];
...
typedef struct __anon_0x197F8 {
    /* 0x0 */ u16 numEntries;
    /* 0x2 */ u8 unpacked;
    /* 0x3 */ u8 pad8;
    /* 0x4 */ enum _GXTlutFmt format;
    /* 0x8 */ char* data;
} __anon_0x197F8; // size = 0xC

typedef struct __anon_0x198C4 {
    /* 0x0 */ struct __anon_0x1959D* textureHeader;
    /* 0x4 */ struct __anon_0x197F8* CLUTHeader;
} __anon_0x198C4; // size = 0x8

typedef struct __anon_0x19935 {
    /* 0x0 */ u32 versionNumber;
    /* 0x4 */ u32 numDescriptors;
    /* 0x8 */ struct __anon_0x198C4* descriptorArray;
} __anon_0x19935; // size = 0xC

typedef struct _GXTexObj {
    /* 0x0 */ u32 dummy[8];
} __anon_0x199F1; // size = 0x20
...
typedef struct _GXColor {
    ...
    /* 0x2 */ u8 b;
    /* 0x3 */ u8 a;
} __anon_0x19A37; // size = 0x4

// Range: 0x80013184 -> 0x80013854
s32 movieGXInit() {
    // Local variables
    s32 i; // r31
...
// Range: 0x80012C68 -> 0x80013184
s32 movieDrawImage(struct __anon_0x19935* tpl, s16 nX0, s16 nY0) {
    // Parameters
    // struct __anon_0x19935* tpl; // r25
    // s16 nX0; // r26
    // s16 nY0; // r27
...
// Range: 0x80012AA4 -> 0x80012C68
s32 movieDrawErrorMessage(enum __anon_0x1900C movieMessage) {
    // Parameters
    // enum __anon_0x1900C movieMessage; // r1+0x8
...
// Range: 0x80012844 -> 0x80012AA4
s32 movieDVDShowError(s32 nStatus) {
    // Parameters
    // s32 nStatus; // r1+0x8

    // Local variables
    enum __anon_0x1900C nMessage; // r31
...
// Range: 0x80012334 -> 0x80012460
static void* Reader() {
    // Local variables
    struct __anon_0x18206* readBuffer; // r27
    s32 offset; // r26
    s32 size; // r25
    s32 readFrame; // r24
